	...
	function beforeSubmit(context)
	{
		...
		if( nRec != null )
		{
			...
			if(LineCount > 0){
				for(var i=0;i<LineCount;i++){
					...
					if(i< Math.round(LineCount/2)){
						var display_I = currentRecord.getSublistValue({
							sublistId: 'recmachcustrecord_13_parent_id(待補)',
				...
				var sales_amt = nRec.getValue("custrecord_12_sales_amt(待補)");
				...
				var vat_io = nRec.getValue("custrecord_12_vat_io(待補)");
			...
			if(!IsError){
				//#region 取得折讓單號
				//稅籍編號
				var registration_number = currentRecord.getValue('custrecord_12_registration_number');
				//折讓簿冊	
				var book_id = currentRecord.getText('custrecord_12_gui_book_id');		
				...
				customrecord_ev_pay_invoices_allSearchObj.run().each(function(result){
					var gui_no = result.getValue({ name: 'custrecord_10_gui_no'
					});
